...
> **마지막 업데이트**: 2026-04-23 (cron sync: 11건)
...
| From | To | Protocol | Action | Message Format |
|---|---|---|---|---|
``
``
...
## Sensors/Vision pipeline ()
포토 센서(Photo Sensor)부터 이미지 캡처, 전송까지의 양방향 데이터 파이프라인으로 비전 시스템(Vision System)의 검사(Inspection) 공정 자동화를 지원. **V7 아키텍처**
...
``
``
| Video Control Service(Jetson) | Management Service | 🟠 TCP ↔ | WatchCameraFrames(Jetson → Management Service)ResumeConveyor(Management Service → Jetson) | Protobuf (JPEG frames) |
``
````
| HW Control Service(ESP32) | Video Control Service(Jetson) | 🔵  ↔ | TriggerSignal (ESP32 → Jetson)/ConveyorCommand(Jetson → ESP32) | ASCII 라인 프로토콜 (단순 문자열) |

### `TriggerSignal`** , **`ConveyorCommand`**(HW Control Service(ESP32) ↔ Video Control Service(Jetson), Serial)**

### `WatchCameraFrames` (Video Control Service(Jetson) → Management Service (카메라 프레임 스트리밍), TCP)
Jetson이 전송

### `ResumeConveyor`** (Management Service → Video Control Service(Jetson) (컨베이어 재개 명령)**
Management Service가 보내는 메세지
Jetson이 응답
Inquiries
...
**최종 수정**: v15 (2026-04-23 sync)

목적: ESP32와 RC522 ,바코드 스캐너 간의 SPI 통신 정상 작동 확인.
내용: Ntag 213 스티커를 태깅했을 때 고유 UID가 시리얼 모니터에 정확히 출력되는지 테스트.
바코드를 스캔했을때 고유 UID가 시리얼 모니터에 정확히 출력되는지 테스트
...
**< 맨홀에 태그 부착 >**
**CASE 1**
NFC 스티커 이용 - 아이템 아이디를 미리 nfc에 삽입해 둬야함
**CASE2**
...
### 실험 2: 바코드를 이용한 HID 통신 (실측 완료)
실측 2026-04-22 · 50회 벤치마크 **100% 정확도 · 평균 45ms** 달성.

#### 배경
RFID (실험 1) 는 SPEC-RC522-001 회귀 스위트 99% / 70 tests 통과로 최종 검증 완료. 대안으로 **Code128 바코드 (HID 키보드 에뮬레이션)** 를 테스트하여 정확도·레이턴시·편의성을 비교한다. 페이로드 포맷은 동일 (`order_{N}_item_{YYYYMMDD}_{M}`) 이라 RFID 경로와 직접 비교 가능.
...
|---|---|---|
````
| Jetson Orin NX | Tailscale 100.77.62.67 (jetson-conveyor) | ssh alias 사용 |
``````
| USB 바코드 리더 | Jetson USB | 0483:0011 STMicroelectronics · HID Keyboard Boot · /dev/input/event5 (by-id: usb-USB_Adapter_USB_Device-event-kbd) · 1D red laser scanner |
````****
| ESP32 (기존) | Jetson USB | 10c4:ea60 CP210x · /dev/ttyUSB0 · v5.0 업로드됨 (바코드 테스트에는 펌웨어 역할 없음) |
데이터 흐름 (RFID 대비):

| 단계 | RFID | 바코드 |
|---|---|---|
| 감지 | RC522 SPI (근접 ≤ 3cm) | HID 레이저 스캔 (10~20cm) |
``
| 호스트 전달 | ESP32 Serial JSON | USB HID 키보드 입력 → /dev/input/event5 |
``
``
| 파싱 | NDEF Text (parseNDEF) | python-evdev 로 KEY_* 조립 |
``
``
| 저장 | rfid_scan_log (Mgmt gRPC) | (예정) 동일 테이블에 reader_id="BARCODE-01" 로 저장 |

#### 준비 도구 (GitHub repo `kiminbean/casting-factory`)

| 파일 | 역할 |
|---|---|
``
| tools/barcode_probe.py | Jetson HID 단일 스캔 진단 (evdev · JSON 출력) |
``
````
| tools/gen_test_barcodes.py | 로컬 Mac 에서 Code128 PNG 5개 생성 (python-barcode + Pillow, module_width 0.4mm) |
``
| tools/gen_test_barcodes_code39.py | Code128 미지원 리더 대체용 Code 39 생성기 (ORDER-N-ITEM-YYYYMMDD-N) |
``
| tools/barcode_benchmark.py | Jetson 반복 스캔 벤치마크 (50회 · 정확도·latency·중복 JSON 리포트) |
``
| docs/barcode_vs_rfid_prep.md | 준비 보고서 (블로커 · 하드웨어 인벤토리) |
``
| docs/barcode_test_plan.md | 실행 플랜 (단계별 체크리스트) |
``
| docs/barcode_jetson_ready.md | 실행 핸드오프 보고서 (권한 해결 3경로 · 벤치마크 명령) |
...
``
``
| 1 | Jetson python-evdev 미설치 | ModuleNotFoundError: No module named 'evdev' | python3 -m pip install --user evdev → v1.9.2 (aarch64 wheel 자동 빌드) |
````
````
``
| 2 | jetson 사용자 input 그룹 미소속 | /dev/input/event5 open 시 PermissionError: [Errno 13] | sudo usermod -aG input jetson + 새 SSH 로그인 (reboot 불필요) |
****
````
````````
| 3 | 바코드 인쇄물을 리더가 디코딩 실패 (beep 없음) | gen_test_barcodes.py 기본값 module_width=0.2mm → 일반 프린터 인쇄 시 실제 바 너비가 리더 해상도 한계 이하 | ImageWriter 옵션 명시: module_width=0.4, quiet_zone=10, dpi=300 → 이후 단일 스캔 45ms 성공 |
``
| 4 | (기록만) arduino-cli Jetson PATH 에 없음 | 바코드 테스트 영향 없음 (리더는 HID, ESP32 불필요) | 조치 불필요 |

#### 실행 단계 (실측 순서)

1. 
로컬 Mac: `python3 tools/gen_test_barcodes.py` → `tools/barcodes_out/*.png` 5개 생성 (1492×348 px @ 300 DPI → 인쇄 시 약 12.6cm × 3cm)

1. 
레이저 프린터로 **실제 크기 100%** 인쇄 (Fit to Page 해제 필수)

1. 
Jetson `scp` + `python3 tools/barcode_probe.py` — 단일 스캔 성공 확인 (45ms, `text` 필드 페이로드 완전 일치)

1. 
나머지 4개 인쇄 후 `python3 tools/barcode_benchmark.py --expected order_1 --expected order_2 ... --reps 10 --timeout 90` → 50회 측정
...
| 정확도 | ≥ 99% | 100.0% (50/50) | ✅ |
****
| 평균 latency | < 500 ms | 45.0 ms | ✅ |
****
| 최대 latency | < 1500 ms | 46 ms | ✅ |
****
| 기대 외 페이로드 | 0 | 0 | ✅ |
``
****
...
#### 벤치마크 결과 JSON 요약 (`result_20260422T030428Z.json`)
**스캔 패턴 관찰**: 사용자는 각 바코드를 9~11회씩 연속 스캔 (역순 `order_5 → 4 → 3 → 2 → 1`). 이는 "같은 바코드 반복 스캔 안정성" 을 더 엄격하게 검증한 결과 — 50회 전부 45~46ms 로 동일 디코딩 (std < 1ms, 사실상 결정론적). 벤치마크 로그의 "MISS" 표기는 스크립트의 엄격 순서 비교(`match = text == expected_at_index`) 때문이며 실패 아님. summary 의 `matches=50` 이 실제 정확도.

#### 샘플 스캔 레코드 (index 1 · probe)

- 
`KEY_LEFTSHIFT + KEY_MINUS` → `_` (언더스코어) 매핑 정상

- 
27개 HID 키 이벤트 → 23자 페이로드 변환 (45ms 소요)

#### RFID vs 바코드 최종 비교 (실측 반영)

****
| 지표 | RFID (NTAG213 + RC522) | 바코드 (Code128 + HID) |
|---|---|---|
****
| 정확도 (%) | 99% (SPEC-RC522-001, 70 tests) | 100.0% (50/50) |
****
| 평균 latency | 100~300 ms (ESP32 → Serial → 호스트 파싱) | 45.0 ms |
****
| 최대 latency | ~500 ms | 46 ms |
****
| latency 표준편차 | 수십 ms | < 1 ms (거의 결정론적) |
| 중복 탐지 | UID 고유 (제조 번호) | 페이로드 기반 (프린트 시 제어) |
****
| 스캔 거리 | ≤ 3 cm | 10~20 cm (시야 확보 시 더 멀리) |
| 오염 내성 | 우수 (RF 관통) | 보통 (광학 · 먼지/긁힘 민감) |
| 방향성 | 무방향 (근접만) | 리더 각도 ±60° |
| 조명 영향 | 없음 | 강한 외광 반사 시 성능 저하 가능 |
****
| 라벨 비용/개 | ~200원 (NFC 스티커) | ~10원 (종이 라벨) |
| 재사용성 | NDEF 재기록 가능 | 폐기 |
| 편의성 | 자동 근접 스캔 | 조작자 시선 확보 + 스캐너 트리거 필요 |
****
| 인쇄 설정 민감도 | 낮음 (전자 태그) | 높음 (module_width · quiet_zone 정확히 설정 필요) |
| 리더 설정 민감도 | 낮음 (UID 표준) | 중간 (symbology 활성화 여부) |
| 현장 고장 시 대응 | 태그·리더 각각 교체 | 리더 교체 · 라벨 재인쇄 (저비용) |
...
- 
**실행자**: kisoo (로컬 Mac) + jetson-conveyor (SSH 자동화)

- 
**리포트 파일**: `tools/barcode_bench_out/result_20260422T030428Z.json` (Jetson 및 Mac 사본)

- 
**총 스캔 시간**: 약 5분 (50회, 사용자 페이스)

- 
**관찰된 이슈**:

  - 
초기 인쇄물은 기본 0.2mm 바 너비로 디코딩 실패 (beep 없음) → 0.4mm 재생성으로 해결

  - 
리더의 `iManufacturer`/`iProduct` 필드가 `"USB Adapter"`/`"USB Device"` placeholder 로 저장되어 실제 브랜드 식별 불가 (STM32 SDK 예제 디스크립터 그대로 사용하는 저가 중국제 OEM 전형 패턴)

  - 
상품 바코드(EAN-13)도 초기 시점엔 미디코딩 → USB 재연결(power cycle) 로 리더 내부 상태 초기화 후 복구

- 
**최종 선택**:

  - 
**1 순위: RFID** (생산 현장 오염·먼지 환경에서 광학 신뢰도 우려, UID 고유성)

  - 
**2 순위: 바코드** (저비용·높은 정확도·빠른 레이턴시, RFID 장애 시 폴백)

  - 
**혼용 전략 검토 가치 있음**: 맨홀에 NFC 스티커 + Code128 바코드 둘 다 부착, 현장 환경별 fallback
...
### 2.1 구성 요소
Pod < Node < Cluster 관계로 이해하면 편함.

- 
Pod: 쿠버네티스에서 컨테이너를 실행하는 최소 단위

- 
Node: 실제 서버. Pod가 실행되는 컴퓨터

- 
Cluster: 전체 쿠버네티스 환경. 여러 대의 Node를 묶어놓은 집합

| 구성 요소 | 설명 |
|---|---|
****
|   | 컨테이너의 최소 실행 단위. 본 프로젝트에서는 Pod 1개 = 모델 1개 |
****
| Node | Pod가 실행되는 물리 머신 (ex: AI Server 1, 2, 3) |
****
| Cluster | Node들의 집합. 전체 쿠버네티스 환경을 의미함 |
****
| Deployment | Pod의 배포 및 업데이트 관리 |
****
| Service | Pod에 접근하기 위한 네트워크 엔드포인트 |
****
| Ingress | Cluster 외부에서 들어오는 요청(HTTP/HTTPS)을 어떤 Service로 보낼지 규칙에 따라 결정하는 역할을 함. |
****
...
Main Server는 단일 엔드포인트만 바라보고, 한다.
...
| Standup | Standup_260422_2팀 |   |   | 제출 |
...
|   |   | Image Classification |   | [부적합] 슬롯형 보관함 위 맨홀 샘플에 대해 이미지 분류 진행 |   |   |   |   |   |
|   |   | Object Detection |   | YOLO 객체 탐지 진행(슬롯형 보관함 위 맨홀 샘플) |   |   |   |   |   |
...
| 구분 | AI Server | 내 PC |
|---|---|---|
| 역할 | PatchCore 개발 + Docker 이미지 빌드 | Docker 이미지 실행 |
| Docker 설치 | ✅ 필요 | ✅ 필요 |
| OS | Ubuntu 24.04 x86_64 | Ubuntu 24.04 x86_64 |
| GPU | GeForce RTX 2080 Ti Rev. A | GeForce RTX 4060 Max-Q / Mobile |
| RAM | 31Gi | 15Gi |

### 2.2 AI Server — 개발 환경 구성
AI Server에서 실행 가능한지 확인한다. 이
해당 문서에서는 **PatchCore 모델(불량 탐지)**을 기준으로 개발 환경을 구성한다.

- 
Tailscale에 연결된 PC에서 `http://100.66.177.119:8080/docs` 주소로 접속하면 사용 가능한 API 엔드포인트, 요청 시 필요한 파라미터와 Body, 요청/응답 예시를 확인하고 직접 API를 테스트할 수 있다.

- 
추론 요청
위 이미지로 추론(불량 검사) 테스트 진행(파일명: `rotated_distortion_0_297.png`)

  - 
1. `/docs` 위에서 설명한 docs 페이지에서 직접 추론 테스트

  - 
2. [Postman](https://www.postman.com/downloads/)을 사용하여 동일하게 API Test를 할 수 있음
(예시: 내 PC에서 Postman을 사용하여 추론 테스트)

  - 
3. curl을 이용한 추론 테스트
(예시: 내 PC에서 curl로 추론 테스트)
...
- 
`FROM`: Docker 이미지를 만들 때 사용할 베이스 이미지를 지정하는 명령어. 로컬(docker images로 확인 가능)에 없는 경우 docker hub에서 받아옴. 
...
이미지이름: `pytorch`
  ...
  - 
태그: `2.11.0-cuda12.8-cudnn9-runtime`

- 
개발 환경(AI Server)과 같은 버전의 pytorch를 설치해준다.

- 
공식 Docker hub([https://hub.docker.com/r/pytorch/pytorch/tags](https://hub.docker.com/r/pytorch/pytorch/tags)) 에 원하는 버전이 없는 경우 cuda, cudnn major version만 맞추고, requirement.txt에 pytorch 관련 패키지를 넣어서 설치.

  - 
예시: CUDA 12.8 → major는 소수점 앞 숲자인 12, minor는 소수점 뒷 숫자를 포함한 12.8
`RUN`: Docker 이미지 안에 필요한 프로그램을 설치하거나 설정을 추가하는 명령어

- 
시스템 레벨의 라이브러리들을 따로 설치해준다.

- 
`rm -rf /var/lib/apt/lists/*`: 패키지 설치 완류 후에는 패키지 목록이 더 이상 필요없고, 그냥 두면 Docker 이미지 크기만 커지기 때문에 제거해줌.

- 
`pip install opencv-python`과 다른점

  - 
`pip`: python에서 `import cv2`를 할 수 있도록 해줌. python 코드와 OpenCV C 라이브러리 연결

  - 
`apt-get`: OpenCV가 실제로 동작하는데 필요한 C 라이브러리
(optional) Docker 컨테이너 내부에 python uv를 사용하기 위해 설치
`WORKDIR`: 컨테이너에서 작업이 실행될 폴더를 설정하는 명령어
`COPY`: 로컬 파일을 Docker 이미지 안으로 복사하는 명령어. 위의 경우 `/app/requirements.txt`로 복사

- 
위에서 `uv`를 설치하지 않을 경우 일반 `pip`로 설치한다(`uv`만 제거)

  - 
`--system`: 시스템 전체 환경에 패키지 설치(가상환경이 아닌 시스템 환경)

  - 
`--break-system-packages`: 시스템 패키지와 충돌 가능성이 있어서 강제로 설치(ubuntu기본값이 pip 설치가 막혀있음)

  - 
`--no-cache-dir`: 설치할 때 캐시를 저장하지 않아서 이미지 용량을 줄임
(optional) `EXPOSE`: Docker 컨테이너가 사용할 포트를 알려주는 명령어(단순 설명용)
`CMD`: 컨테이너가 시작될 때, 기본으로 실행할 명령어를 지정한다. 위의 경우 컨테이너가 실행되면 `uvicorn server:app --host 0.0.0.0 --port 8080` 명령어가 실행된다.
아래는 Docker 이미지를 빌드하는데 필수로 있어야 하는 파일이다.
AI Server 개발 환경을 Docker 이미지로 패키징한다. 아래와 같이 3가지 파일이 존재하는 경로에서 빌드해야 함

- 
`Line 2`

  - 
`docker build -t {registry}/{image_name}:{tag}`와 같이 빌드하면 `docker tag`를 따로 해줄 필요가 없고, `docker push`만 하면 됨

    - 
(임의 설정 불가능) `registry`: addteam2

      - 
Github Repo와 비슷한 개념

    - 
(임의로 설정 가능) `image_name`: ai-server

    - 
(임의로 설정 가능) `tag`: v1.0

### 2.4 내 PC — Docker 설치
Engine, Desktop중 하나 설치

- 
Docker Engine(on Ubuntu 24.04): [https://docs.docker.com/engine/install/ubuntu/](https://docs.docker.com/engine/install/ubuntu/)

- 
Docker Desktop(on Ubuntu 24.04): [https://docs.docker.com/desktop/setup/install/linux/ubuntu/](https://docs.docker.com/desktop/setup/install/linux/ubuntu/)
...
#### **3.2.1 AI Server — 이미지 전달**

- 
`Line 8`

  - 
`docker build`할 때, 이름을 `{registry}/{image_name}:{tag}`로 설정해주면, 따로 `docker tag`를 안해줘도 됨.

- 
`Line 9`

  - 
`Registry`가 지정된 이미지만 push할 수 있음. 예시에서는 `Registry=addteam2`

#### **3.2.2 내 PC — 이미지 실행**
**Docker Login**

- 
team2 공용 Google 계정(`addineduteam2`)으로 로그인 → 터미널에 Login Succeeded 떠야함
`docker-composer.yaml`** 작성 (파일명 고정)**
...
experimental 저장소는 “아직 완전히 검증되지 않은 최신 기능/버전”을 제공하는 곳이라 기본적으로 비활성화(주석)되어 있다
Docker Conatiner에서 NVIDIA runtime을 사용할 수 있도록 설정
**Docker 환경 구성 및 추론 테스트**

- 
`Line 5`: AI Server에서 push한 이미지를 그대로 pull 해야 함

- 
`Line 8`: `docker-compose.yaml` 파일이 존재하는 디렉토리에서 실행 해야 함

- 
`Line 12`: 내 PC에서도 동일하게 Postman, curl, `/docs`를 이용해서 추론 요청 → 결과 응답을 확인할 수 있음.

### 3.3 결과

| 항목 | AI Server | 내 PC (Docker) |
|---|---|---|
...
의존성 문제를 AI Server에서 해결하여 `requirements.txt`로 저장하고 `Dockerfile`에 등록하여 이미지 빌드를하면, 내 PC에서는 `docker compose up`명령어로 손쉽게 AI Server와 동일 환경을 세팅할 수 있다.

- 
내 PC 추론 결과(postman 이용)
...
### 4.1 목적
AI Server 프로세스가 강제 종료되었을 때, AI Server와 Docker 환경에서의 복구 과정을 비교한다.
...
| 항목 | AI Server (개발 환경) | 내 PC (Docker) |
|---|---|---|
...
### 5.1 목적
새로운 불량 탐지 모델로 교체할 때, AI Server와 Docker 환경의 배포 절차를 비교한다.
...
- 
다른 버전의 모델로 교체 했을때의 결과(postman 이용) → 현재 원형 맨홀에 대한 모델만 있어서, 다른 데이터로 학습된 모델로 변경했음.
...
### 1.1 목적
AI Server 3대에 Kubernetes를 적용하여, 서버 장애 상황에서도 모든 모델 추론이 중단 없이 유지되는지 확인한다. → 현재 서버가 2대 밖에 없으므로 2대에서 테스트 진행.
...
### 2.1 설치
K8s를 사용하려는 Server에 모두 설치해야 한다.

- 
`containerd(Docker Engine)` 설치: [https://docs.docker.com/engine/install/ubuntu/](https://docs.docker.com/engine/install/ubuntu/)

- 
`kubectl`, `kubeadm`, `kubelet` 설치: [https://kubernetes.io/docs/setup/production-environment/tools/kubeadm/install-kubeadm/#installing-kubeadm-kubelet-and-kubectl](https://kubernetes.io/docs/setup/production-environment/tools/kubeadm/install-kubeadm/#installing-kubeadm-kubelet-and-kubectl)

### 2.1 서버 구성

| 구분 | AI Server 1(100.66.177.119) | AI Server 2 | AI Server 3(100.95.62.52) |
|---|---|---|---|
| OS | Ubuntu 24.04 x86_64 | Ubuntu 24.04 x86_64 | Ubuntu 24.04 x86_64 |
| GPU | GeForce RTX 2080 Ti Rev. A | GeForce RTX 2080 Ti Rev. A | GeForce RTX 4060 Max-Q / Mobile |
| RAM | 31Gi | 31Gi | 15Gi |
| K8s 역할 | Control PlaneWorker Node 1 | Worker Node 2 | Worker Node 3 |
`Control Plane`: K8s 클러스터 전체를 관리하는 컴포넌트. 실제 Pod을 실행하지 않고 관리만 담당
`Worker Node`: 실제 Pod가 실행되는 서버. Control Plane의 지시를 받아 실행만 담당
...
### 2.5 사전 준비 — 모든 서버 공통
AI Server 1, 2, 3 모두에서 아래 작업을 순서대로 진행한다.

#### Step 01. swap 비활성화
K8s는 swap이 활성화되어 있으면 정상 동작하지 않는다.

#### Step 02. containerd 설정
Docker 설치 시 containerd가 함께 설치되지만, K8s와 연동하려면 추가 설정이 필요하다.

### 2.6 클러스터 초기화 — Control Plane에서만 실행

#### Step 03. Control Plane 초기화

- 
`<CONTROL_PLANE_IP>`: tailscale ip를 입력해준다.(AI Server 1: `100.66.177.119`)

#### Step 04. kubectl 설정

#### Step 05. 네트워크 플러그인 설치 (Flannel)
Pod 간 통신을 위한 네트워크 플러그인을 설치한다.

### 2.7 Worker Node 참여 — Worker Node에서 실행

#### Step 06. 클러스터 참여
Worker Node에서 각각 실행한다. **Step 4에서 복사해둔 join 명령어를 사용**한다.

#### Step 07. 노드 참여 확인
Control Plane에서 확인한다.

- 
`addinedu-katane-17-b13vfk`: AI Server 3 (Worker Node)

- 
`team2`: AI Server 1 (Control Plane & Worker Node)

### 2.8 이미지 빌드 및 배포

#### Step 08. Docker 이미지 빌드 및 각 서버에 전달
K8s는 각 노드에서 이미지를 직접 가져온다. Registry가 없는 경우 각 서버에 이미지를 직접 로드한다.

#### Step 09. GPU 플러그인 설치 — Control Plane에서만 실행
K8s가 GPU를 인식하려면 NVIDIA Device Plugin이 필요하다.

#### Step 10. Deployment 적용 — Control Plane에서만 실행
2.4에서 작성한 Deployment yaml 파일을 적용한다.

#### Step 11. Service 적용 — Control Plane에서만 실행
모델 단위 Service의 장점
kubernetes는 K8s가 자동으로 만드는 기본 Service.

### 2.9 동작 확인 — 실험 전 정상 상태 검증
실험을 시작하기 전에 모든 모델이 정상적으로 추론 요청을 처리하는지 확인한다.
모든 요청에서 정상 응답이 오면 실험을 시작한다.
...
## 전체 구조
Fallback 상황에서도 Redis가 두 Pod 간의 추론 순서를 조율해줘서 OOM 없이 안전하게 운영 가능해.
