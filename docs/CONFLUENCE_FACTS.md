...
> **마지막 업데이트**: 2026-04-19 (cron sync: 162건)
...
> 2026-04-19 업데이트: homepage **753829** 하위 총 **217 페이지** 중 **65 페이지** (technical/architectural content 위주) 를 팩트 소스로 추적한다. standup / 회의록 / 멘토링일지 / sprint 회의 / Forms & Templates 등 **전달성 자료는 의도적으로 제외**했다. 상세 제외 정책은 § 미포함 카테고리 참조.
...
`addinedute` space homepage **753829** 하위 총 **217 페이지** 중, 본 팩트 인덱스는 **65 페이지**(technical/architectural content)만 추적한다. 다음 카테고리는 의도적으로 제외되었다.

| 카테고리 | 대표 페이지 ID 패턴 | 제외 사유 |
|---------|---------------------|----------|
| Standup 일지 | `Standup_2026MMDD` (15+ 페이지) | 일일 진행 상황 — 코드 교차검증 가치 낮음 |
| 회의록 | `회의록_2026MMDD_*` (6+ 페이지) | 회의 메모 — 결정사항이 §1·§2 의 design/scenarios 로 이미 흡수됨 |
| 멘토링일지 | `멘토링일지_260*` / `멘토링일지_2026*` (9+ 페이지) | 학습 노트 — 영구 facts 아님 |
| Sprint 회의/발표 | `sprint{N}_meeting`, `sprint{N}_presentation` (10+ 페이지) | 진행 보고 — 본 인덱스 범위 밖 |
| Forms & Templates (851969) | `회의록_20260000`, `Sprint_20260000` 등 | 빈 템플릿 |
| Notice (3768381), 자료 공유 (2162711) | 1-2 페이지 | 운영 공지 |
| Whiteboard 타입 | `5505054`, `6914067`, `2162773` 등 (5 페이지) | 그래픽 위주, REST API/storage 변환 비호환 |
| Drafts | `5144703 Untitled page`, `786449 테스트` | placeholder/draft |
| 0_Project_Topic_Selection 하위 시연 (4096056, 9568288, 5899172, 5768077, 11600284 등) | 사형 주조 1차/2차/3차 등 | §4 의 `사형 주조 주제 가능성 검증 테스트 (3407954)` 가 상위 요약 보유 |
| 06_Deliverables (3113021) 전반 | Presentations, Demo Videos, Reports | 외부 제출물 — 본 코드 교차검증과 무관 |
| 05_Meetings_and_Logs (3407968) | Meeting Notes, Standup, Work Logs, Mentoring Logs | Standup/회의록 카테고리에 포함 |

**필요 시 추가 방법**: 위 카테고리에서 특정 페이지를 추적하고 싶으면, 해당 섹션에 `### 페이지 제목 (PAGE_ID)` 형식 헤더를 수동 추가한 뒤 `python3 scripts/sync_confluence_facts.py` 실행 → 자동으로 본문 fetch + commit.

---

## 9. 전체 페이지 (추적 확장)

> 2026-04-19: 사용자 요청으로 homepage 753829 하위 **모든 페이지**를 팩트 인덱스에 등록.
> Standup / 회의록 / 멘토링일지 / Sprint / 실험 sub-pages / Forms & Templates 포함.
> Whiteboard 타입은 v2 API storage format 미지원으로 본문 없이 URL 만 기록.
...
| Standup | Standup_260417_2팀 |   |   | 제출 |
...
| Sprint Presentation | 2팀_발표자료_Sprint3 |   |   |   |
...
낮은 고정 장애물은 LiDAR로 감지할 수 없기 때문에, 출입 금지 구역을 설정하기 위한 실험
...
# 발표용 정리..?
정확한 Localization과 Navigation의 안정적인 주행 성능을 위해 고품질의 Map을 만드려고 노력했고,
**odometry의 이상을 감지하고**

- 
wheel radius를 조정하였으며
**map의 계단 현상을 완화하기 위해**

- 
mapping 전에 로봇을 map의 벽과 축을 맞춰 정렬하였고
**map의 표현과 navigation 주행을 세밀하게 하기 위해  **

- 
resolution 0.025로, cell당 2.5cm로 설정했다.
향후 Keepout filter를 설정하여, 출입 금지 구역을 설정하기 위한 실험을 진행할 예정이다.
...
`footprint_padding: 0.0`
  ...
  - 
0.0으로 설정 → 로봇의 실제 크기만큼만 root footprint 
...
### **목표 도달**
최종적으로 목표에 도달했을 때의 조건 (목표에 어느 정도 이내& 방향 threshold) 에 도달하면, 도착으로 판단 ** 관련**
...
# 
[https://dayelee313.atlassian.net/wiki/x/KYG1](https://dayelee313.atlassian.net/wiki/x/KYG1) 
...
- 
covariance 설정이 어렵지만 잘 할 수 있다면 사용하는 것도 좋다고 말씀해주심

# 발표용 정리..?

### MAP
가장 좁은 통로를 0.24m로 가정하고,
좁은 공간에서 안정적인 경로를 계획과 주행할 수 있도록

- 
controller의 최대 속도를 0.1로 설정

- 
costmap의 footprint_padding, resolution 및 inflation 설정을 통해

- 
obstacle의 세밀한 표현을 위해 resolution을 0.01로 설정하였고

- 
정밀한 Localization을 위해

- 
amcl의 update frequency의 주기를 단축시키고,
목적지까지의 정밀한 도착을 보장하기 위해

- 
goal checker의 goal tolerance를 축소하였으며,

- 
향후 aruco marker를 도입하여 완벽한 도착을 보장할 예정이다. 
연산량을 줄이기 위해

- 
필요한 만큼의 local costmap 크기를 조절하였다.
파라미터 튜닝을 아직 진행하고 있다!!
...
# 조사 목적
파라미터 조정에 참고하기 위해 nav2 파라미터를 조사한다.

# 조사 개요 
이를 위해 RPP, Costmap, AMCL의 파라미터들을 조사하였다.
...
# 결과
프로젝트의 물리적인 환경과 AMR의 물리적인 한계에서 프로젝트 목표를 달성 할 수 있는 파라미터 목록을 정리해냈다.
...
- 
정밀 접근 전 서야 하는 위치, 대략 목표 위치 2~30cm 전
**dock pose**

- 
로봇이 도킹할 위치, `use_external_detection_pose`파라미터를 사용함에 따라 두 가지로 나뉨
**use_external_detection_pose**: false

- 
기본적으로 주어지는 `dock_pose`를 믿고 해당 위치로 도킹
**use_external_detection_pose**: true

- 
외부 센서(카메라, april tag)등을 통해서 `dock_pose`를 재계산 후 도킹
**Refined pose**: 

- 
use_external_detection_pose가 true라면, 외부 센서를 통해서 보정된 정밀한 `dock_pose` 

- 
use_external_detection_pose가 false라면 설정한 `dock_pose` 그대로 사용
**April tag pose:**
docking server plugin이 아닌 카메라+april tag의 위치를 검출하는 외부 노드가 발행하는 pose 정
...
# Docking Server 구성 & 동작 방식
Docking server는 nav2의 node, 내부적으로 크게 세 축으로 나뉜다.
**간략하게 보면**

- 
plugin은 pose/상태를 제공하고

- 
controller는 속도를 계산하고

- 
docking server가 그 속도를 publish
**DockingServer**
전체를 orchestration

- 
액션 처리, 전체 순서 제어, staging 이동 호출, retry, feedback, cmd_vel publish 담당
**Dock plugin**
로직 담당
...
- 
staging pose 계산, 센서 기반 dock pose 보정, `isDocked`, `isCharging`등 도착 판정
**Controller**
목표 pose로 가려면 어떤 cmd_vel이 필요한가?
refined target pose를 따라갈 속도 계산과 충돌 체크 담당
**조금 더 깊게 보면**

- 
외부 detector가 AprilTag pose를 계산하고

- 
dock plugin이 그걸 `map/odom` 기준의 refined dock pose로 바꾸고

- 
docking server가 그 pose를 다시 로봇 기준(`base_link`)으로 변환해서

- 
controller가 그 상대 오차로 `cmd_vel`을 계산한다

# [코드로 알아보자](https://github.com/ros-navigation/navigation2/blob/1.3.10/nav2_docking/opennav_docking/src/docking_server.cpp#L202-L360)

## 전체 프로세스
**#226~239**: dock id를 사용하면, dock_db에서 id를 통해 pose와 orientation을 얻어오기
**#241~248**: 도킹 시작 전에 이미 도킹이 되어있는지 체크
**#250~269**: 현재 Staging pose 위치가 아니라면, nav2로 Staging pose까지 이동
**#271~273**: 우리가 설정한 dock pose를 docking_server.fixed_frame에 설정한 frame 좌표계로 변환
**#276**:
doInitialPerception()은 docking을 시작하기 전에 우리가 설정한 external pose를 refined pose로 변환

- 
docking을 시작하기 전에 최소한 로봇이 aruco marker를 보고 있는지 확인하기 위해!!

- 
refined pose가 없으면, docking action 실패

- 
최소한 docking 시작 전에는 aruco marker가 보이는 pose로 이동해야함.
**#279~325**:

1. 
approachDock()을 반복 호출하면서 refined pose를 계속 갱신하고

1. 
그 pose를 따라 controller가 cmd_vel을 계산

1. 
plugin에서 도킹이 되었는지, 충전 중인지 확인하여 docking 성공 여부를 판단하고

1. 
실패하면 staging 쪽으로 다시 물러난 뒤 retry 횟수만큼 재시도함
참고 : [Docking server document](https://docs.nav2.org/tutorials/docs/using_docking.html), [Nav2 Docking server github](https://github.com/ros-navigation/navigation2/tree/1.3.10/nav2_docking/opennav_docking)

# 실험 1. 기본 plugin
SimpleNonChargingDock이라는 기본 플러그인으로 테스트
action으로 넣어준 map frame 기준 dock_pose로 이동
근접 도킹 루프에서 사용되는 refined pose는 external detection(April tag)가 꺼져있어서 입력된 dock pose 그대로 반환
docking server가 그 목표 pose를 로봇 기준으로 변환해서 controller에 넘김
`controller`는 그 상대 목표를 따라갈 `cmd_vel`을 계산하고, 충돌 검사도 함께 합니다.
`docking_server`가 계산된 속도를 publish
지금 launch에서는 `cmd_vel -> cmd_vel_nav`로 remap해서 기존 velocity_smoother로 입력되게.
내가 지정한 map상의 pose 앞까지 간 뒤, 마지막에 그 pose에 가까워지면 성공 처리하는 docking 흐름
[시연 영상](https://drive.google.com/file/d/1vg2jXsDDRxLheDAUijNwY0vnEjuN6ITG/view?usp=drive_link)

# 실험 2. April tag를 외부 센서로 사용
...
### 질문은
| 1. 캘리브레이션 사진을 보여드리면서 이런 사진을 사용하는 게 맞나요?
| 2. 핑크랩에서는 docking server를 사용하셨는지?

### 민경환 강사님의 대답은
| 1. [calibration 강의 자료](https://github.com/pinklab-art/pinky_study/wiki/2.2-Pinky-Pro(part2))에 있는 체커보드를 사용해보기
| 2. Nav2 스택 말고 직접 개발해서 사용했다.
...
# 설명
Nav2의 설계 범위로는 해결할 수 없는 문제를 별도의 제어 계층에서 처리하기 위해서 Docking Server가 필요하다. ArUco Marker와 연계하여 스테이징 자세에서 보다 더 정교한 위치조정이 가능하다.
...
#### PostgreSQL 데이터베이스 연동 및 쿼리 실행 실험 (31260684)
...
#### (미완성)[Detection] YOLO Object Detection with manhole mock-up image dataset on slotted storage(슬롯형 보관함) (31064096)
...
mAP(mean Average Precision)
  ...
  - 
AP(Average Precision): Precision-Recall 곡선 아래의 면적으로, 모델이 어떤 Recall값에서 얼마나 좋은 즉, 한 개 클라스의 평균 정밀도이다.
...
#### (~ing) Docker 구현 및 테스트 (33521698)

**Confluence URL**: https://dayelee313.atlassian.net/wiki/spaces/addinedute/pages/33521698
**최종 수정**: v4 (2026-04-19 sync)

작성일: 
목적: AI Server에 Docker를 적용했을 때와 직접 설치했을 때의 차이를 실험으로 확인
선행 문서: [https://dayelee313.atlassian.net/wiki/x/awLWAQ](https://dayelee313.atlassian.net/wiki/x/awLWAQ) 

## 1. 테스트 개요

### 1.1 목적
AI Server에 필요한 환경을 직접 설치하는 방식(AI Server)과 Docker로 실행하는 방식(내 PC)을 직접 비교하여, Docker가 실제로 어떤 차이를 만들어내는지 확인한다.  
두 머신의 하드웨어 스펙이 다르므로 **성능 측정은 제외**하고 환경 구성 및 운영 편의성 중심으로 비교한다.
본 테스트는 개발 단계에서 Docker의 효과를 검증하며,  
운영 단계에서는 Kubernetes를 함께 사용한다. (kubernetes_research.md 참고)

### 1.2 검증 항목

| 실험 | 검증 내용 | Docker 기대 효과 |
|---|---|---|
| 실험 1 | 환경 구성 재현성 | 스펙이 다른 머신에서도 동일한 환경 구성 |
| 실험 2 | 크래시 자동 복구 | 프로세스 크래시 시 자동 재시작 |
| 실험 3 | 모델 파일 교체 배포 | 컨테이너 재빌드 없이 모델만 교체 |
| 실험 4 | 의존성 충돌 방지 | 라이브러리 버전 충돌 없이 실행 |

### 1.3 테스트 대상 서버
AI Server를 대상으로 테스트한다. AI Server는 CUDA, PyTorch 등 의존성이 가장 복잡하여 Docker 효과가 가장 극명하게 드러나는 서버다.
...
|---|---|---|
| 방식 | 네이티브 설치 | Docker |
| OS | (실제 OS 기재) | (실제 OS 기재) |
| GPU | (실제 GPU 기재) | (실제 GPU 기재) |
| RAM | (실제 RAM 기재) | (실제 RAM 기재) |
두 머신의 하드웨어 스펙이 다르더라도 본 테스트의 검증 항목(환경 구성, 운영 편의성)에는 영향을 주지 않는다.

### 2.2 AI Server — 네이티브 설치 절차

### 2.3 내 PC — Docker 설치 절차

## 3. 실험 1 — 환경 구성 재현성

### 3.1 목적
스펙이 다른 두 머신에서 동일한 AI Server 환경을 구성할 때의 차이를 비교한다.
...
- 
[ ] AI Server에서 CUDA 버전 충돌이 발생했는가

- 
[ ] 내 PC에서 동일한 이미지로 오류 없이 실행되었는가

- 
[ ] 내 PC의 환경 구성 절차가 AI Server보다 단순한가

## 4. 실험 2 — 크래시 자동 복구

### 4.1 목적
AI Server 프로세스가 강제 종료되었을 때 두 방식의 복구 과정을 비교한다.
...
| 재시작 소요 시간 | 초 | 초 |
| 개발자 개입 필요 여부 | Y / N | Y / N |

### 4.4 체크포인트

- 
[ ] AI Server에서 크래시 발생 시 개발자가 직접 개입했는가

- 
[ ] 내 PC에서 자동으로 재시작되었는가

- 
[ ] 내 PC의 재시작 시간이 AI Server보다 빠른가

## 5. 실험 3 — 모델 파일 교체 배포

### 5.1 목적
새로운 불량 탐지 모델로 교체할 때 두 방식의 배포 절차와 서비스 중단 시간을 비교한다.
...
- 
| 컨테이너 재빌드 필요 여부 |   | Y / N |
| 롤백 용이성 | 어려움 / 보통 / 쉬움 | 어려움 / 보통 / 쉬움 |

### 5.4 체크포인트

- 
[ ] 내 PC에서 컨테이너 재빌드 없이 모델만 교체되었는가

- 
[ ] AI Server 대비 내 PC의 교체 절차가 단순한가
...
### 6.1 목적
AI Server에 필요한 라이브러리를 설치할 때 두 방식의 의존성 충돌 발생 여부를 비교한다.
...
- 
[ ] AI Server에서 의존성 충돌이 발생했는가

- 
[ ] 내 PC에서 동일한 충돌이 발생하지 않았는가

- 
[ ] Dockerfile의 버전 고정이 충돌 방지에 효과적이었는가

## 7. 결과 종합

### 7.1 실험별 결과 요약

| 실험 | AI Server (네이티브) | 내 PC (Docker) | Docker 우위 |
|---|---|---|---|
| 환경 구성 재현성 |   |   | Y / N |
| 크래시 자동 복구 |   |   | Y / N |
| 모델 교체 배포 |   |   | Y / N |
