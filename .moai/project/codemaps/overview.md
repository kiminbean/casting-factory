...
> **Last updated**: 2026-04-14 (V6 아키텍처 8단계 마이그레이션 완료)
...
**V6: Dual Server Process + 4 통신 채널**

```
┌──────────────────────────────────────────────────────────────┐
│  Presentation Layer                                          │
│  ┌────────────────────┐  ┌─────────────────────────────────┐ │
│  │  Next.js 16 Web    │  │  PyQt5 Desktop (Python 3.12)    │ │
│  │  React 19 · TS     │  │  native desktop app             │ │
│  │  localhost:3000     │  │                                 │ │
│  └─────────┬──────────┘  └──────────────────┬──────────────┘ │
│            │ HTTP /api/*                    │ gRPC TCP       │
├────────────┼────────────────────────────────┼────────────────┤
│  Server Layer (V6: 2 프로세스)                                │
│  ┌─────────▼──────────┐  ┌──────────────────▼──────────────┐ │
│  │  Interface Service │  │  ★ Management Service           │ │
│  │  FastAPI :8000     │◄─┤  gRPC :50051                    │ │
│  │  routes/, schemas  │  │  task_manager / allocator /     │ │
│  │  websocket /ws/... │  │  traffic / executor / monitor   │ │
│  └─────────┬──────────┘  └──┬───────────────────────────┬───┘ │
│            │ SQLAlchemy     │ adapters/                 │     │
│            │                │  ros2_adapter (lazy)      │     │
│            │                │  mqtt_adapter (ESP32)     │     │
...
┌──────────────────────────────────────────────────────────▼─────┐
│  HW Layer                                                       │
│  ROS2(AMR/Cobot) | MQTT(ESP32) | gRPC streaming(Image)         │
└─────────────────────────────────────────────────────────────────┘
```

V6 핵심: PyQt 가 Interface 우회 → SPOF 제거. ESP32/AMR/Cobot 별 채널 분리.
