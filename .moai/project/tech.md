...
- **SQLite**: `DATABASE_URL` 미설정 시 `backend/casting_factory.db` 로 폴백
- `database.py:_load_env_local()` 이 stdlib 로 `.env.local` 파싱
- `main.py lifespan` 에서 SQLite 일 때만 DB 파일 삭제·재시드, PG 일 때는 `create_all + idempotent seed` 만 수행
...
| 파일 | 역할 | 실행 |
|---|---|---|
| `backend/scripts/migrate_sqlite_to_pg.py` | SQLite → PG 데이터 이관 | 일회성 (2026-04-09) |
...
3. **PostgreSQL 16 전환** (2026-04-09): SQLite 폴백 유지하되 운영은 PG. `.env.local` 의 `DATABASE_URL` 한 줄로 전환.
...
8. **JSON 컬럼 전략**: diameter_options, thickness_options, materials 등은 `TEXT` 컬럼에 JSON 문자열로 저장 + `ProductResponse.from_orm_model()` 이 파싱. PG 의 `jsonb` 미사용 (SQLite 폴백 호환).
